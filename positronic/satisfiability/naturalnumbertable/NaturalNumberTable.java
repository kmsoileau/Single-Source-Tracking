...
public class NaturalNumberTable implements INaturalNumberTable
{
  ...
  public NaturalNumberTable(String name, int rows, int columns) throws Exception
  {
  	...
  	for(int i=0;i<rows;i++)
  		for(int j=0;j<columns;j++)
  			this.data[i][j]=new NaturalNumber(name+"$"+i+"$"+j);
  }
  ...
  {
    if(this.getNumberOfRows()!=n.getNumberOfRows()) return false;
    if(this.getNumberOfColumns()!=n.getNumberOfColumns()) return false;
    return true;
  }
