...
import positronic.satisfiability.exceptions.BitStringLookupTableEntryerException;
...
public class BitStringLookupTableEntryer extends Problem implements IProblem
{
  private static final long serialVersionUID = 2052275275611995881L;

	public BitStringLookupTableEntryer(IBitString Y, IBitStringLookupTable table) throws Exception
  {
		if(table==null) 
    	throw new BitStringLookupTableEntryerException("A null IBitStringLookupTable was passed to a constructor.");
    if(table.getLength()==0) 
    	throw new BitStringLookupTableEntryerException("An empty IBitStringLookupTable was passed to a constructor.");
    
    this.setClauses(new BitStringLookupTabler(new BitString(Y.size()),Y,table).getClauses());
  }
}
