...
/*
 * BitStringLookupTableInverter.java	1.01 09/06/19
 *
...
import positronic.satisfiability.exceptions.BitStringLookupTableInverterException;

/**
 * <p>Title: <tt>BitStringLookupTableInverter</tt> class</p>
...
public class BitStringLookupTableInverter extends Problem implements IProblem
{
  private static final long serialVersionUID = 3628451045217484035L;

	public BitStringLookupTableInverter(IBitString X, IBitString Y, IBitStringLookupTable table) throws Exception
  {
		if(table==null) 
    	throw new BitStringLookupTableInverterException("A null IBitStringLookupTable was passed to a constructor.");
    if(table.getLength()==0) 
    	throw new BitStringLookupTableInverterException("An empty IBitStringLookupTable was passed to a constructor.");
    
    this.setClauses(new BitStringLookupTabler(Y,X,table).getClauses());
  }
}
