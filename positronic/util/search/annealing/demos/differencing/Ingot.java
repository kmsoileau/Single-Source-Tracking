	...
	public double evaluate()
	{
		...
		for(int i=0;i<this.values.size();i++)
			if(this.values.get(i) instanceof Long)
				pass.add(this.values.get(i).longValue());
			else
				pass.add(this.values.get(i));
		for(int i=0;i<pass.size();i++)
			maxvalue=Math.max(maxvalue,pass.get(i));
		
		ArrayListSet<?> expressible=Expressible.expressibleList(pass);
		Long[] sortarray=(Long[])expressible.toArray(new Long[0]);
		Arrays.sort(sortarray);
		ArrayListSet<Long> dum=new ArrayListSet<Long>();
		/*for(int i=0;i<pass.size();i++)
